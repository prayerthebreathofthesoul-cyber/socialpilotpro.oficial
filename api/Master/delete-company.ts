...
function isIgnorableAuthDeleteError(status: number, error: any) {
...
async function deleteAuthUser(
  ...
  if (
    ...
    !isIgnorableAuthDeleteError(deleteUserResponse.status, deleteUserData)
  ) {
    throw new Error(
      deleteUserData?.message ||
        deleteUserData?.error ||
        "Erro desconhecido ao excluir usuário do Auth."
    );
  }
}
...
export default async function handler(req: any, res: any) {
  try {
    ...
    const postRows = await supabaseGet(
      supabaseUrl,
      serviceRoleKey,
      `/rest/v1/posts?company_id=eq.${encodeURIComponent(
        companyId
      )}&select=id`
    ).catch(() => []);

    const postIds = Array.isArray(postRows)
      ? postRows
