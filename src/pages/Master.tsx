...
function assertCompanyUpdated(updatedCompany: any) {
  if (!updatedCompany?.id) {
    throw new Error(
      "O Supabase não alterou nenhuma empresa. Verifique a permissão UPDATE/RLS da tabela companies."
    );
  }
}

function assertActionApplied(action: string, updatedCompany: any) {
  ...
  if (
    action === "reset_usage" &&
    Number(updatedCompany.posts_used ?? updatedCompany.postsUsed ?? 0) !== 0
  ) {
    throw new Error("O uso mensal não foi zerado no banco de dados.");
  }
}

async function readApiResponse(response: Response, endpointName = "API") {
  ...
  if (response.status === 404 || text.toLowerCase().includes("page")) {
    throw new Error(
      `A API ${endpointName} não foi encontrada no servidor. Publique o arquivo correspondente antes de usar esta ação.`
    );
  }
...
function getStorePatchFromCompany(company: any): Partial<MasterStore> {
...
export default function Master() {
  ...
  async function runMasterCompanyAction(store: MasterStore, action: MasterAction) {
    ...
    throw new Error("Ação master inválida.");
  }
  ...
  async function activatePremium(store: MasterStore) {
    ...
    setRunningAction(getActionKey(store, "activate_premium"));

    try {
      const updatedCompany = await runMasterCompanyAction(
        store,
        "activate_premium"
      );

      updateStoreLocal(store.id, getStorePatchFromCompany(updatedCompany));

      toast.success("Plano Premium ativado.");
      await loadSupabaseCompanies();
    } catch (error: any) {
      console.error(error);
      toast.error(error?.message || "Não foi possível ativar o Premium.");
    } finally {
  ...
  async function cancelPremium(store: MasterStore) {
    ...
    setRunningAction(getActionKey(store, "cancel_premium"));
    ...
    try {
      ...
      console.error(error);
      toast.error(error?.message || "Não foi possível cancelar o Premium.");
    } finally {
      setRunningAction(null);
    }
  }
  ...
  async function blockStore(store: MasterStore) {
    ...
    try {
      const updatedCompany = await runMasterCompanyAction(store, "block");
      ...
      console.error(error);
      toast.error(error?.message || "Não foi possível bloquear a conta.");
    } finally {
      setRunningAction(null);
    }
  }
  ...
  async function unblockStore(store: MasterStore) {
    ...
    try {
      const updatedCompany = await runMasterCompanyAction(store, "unblock");
      ...
      console.error(error);
      toast.error(error?.message || "Não foi possível ativar o usuário.");
    } finally {
      setRunningAction(null);
    }
  }
  ...
  async function resetUsage(store: MasterStore) {
    setRunningAction(getActionKey(store, "reset_usage"));

    try {
      const updatedCompany = await runMasterCompanyAction(store, "reset_usage");
      ...
      console.error(error);
      toast.error(error?.message || "Não foi possível zerar o uso mensal.");
    } finally {
      setRunningAction(null);
    }
  }
            ...
            {isLoadingCompanies ? (
              ...
              <div className="space-y-4">
                {filteredStores.map((store) => {
                  ...
                  const activatingPremium = isRunningAction(
                    store,
                    "activate_premium"
                  );
                  ...
                  const resettingUsage = isRunningAction(store, "reset_usage");
